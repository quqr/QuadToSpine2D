...
using IncrementalValueProviderExtensions = Microsoft.CodeAnalysis.IncrementalValueProviderExtensions;
using SourceProductionContext = Microsoft.CodeAnalysis.SourceProductionContext;
using SymbolDisplayFormat = Microsoft.CodeAnalysis.SymbolDisplayFormat;
using SymbolDisplayGlobalNamespaceStyle = Microsoft.CodeAnalysis.SymbolDisplayGlobalNamespaceStyle;
using SymbolDisplayMiscellaneousOptions = Microsoft.CodeAnalysis.SymbolDisplayMiscellaneousOptions;
using SymbolDisplayTypeQualificationStyle = Microsoft.CodeAnalysis.SymbolDisplayTypeQualificationStyle;
...
public class SourceGeneratorWithAttributes : IIncrementalGenerator
{
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        var provider = IncrementalValueProviderExtensions.Collect(IncrementalValueProviderExtensions.Where(context
            .SyntaxProvider
            .CreateSyntaxProvider(
                static (s, _) => IsSyntaxTargetForGeneration(s),
                static (ctx, _) => GetSemanticTargetForGeneration(ctx)), static m => m is not null));

        context.RegisterSourceOutput(
            IncrementalValueProviderExtensions.Combine(context.CompilationProvider, provider),
        ...
        SourceProductionContext context)
    {
        ...
        foreach (var group in registrationsByLifetime)
        {
            ...
            registrationLines.AddRange(group.OrderBy(r => r.FullName)
                .Select(reg => $"services.{method}<{reg.FullName}>();"));
        }

        // 生成最终源代码
        var source = $$"""
                       // <auto-generated/>
                       // 此文件由 ServiceSourceGenerators 生成：{{DateTime.UtcNow:yyyy-MM-dd HH:mm:ss}} UTC
